...
int ImageMarbleDetector::segmentCOM(Mat* img, int xsegments , int ysegments )
{
	int** segments = NULL;
	segments = new int* [xsegments];
	for (int i = 0; i < xsegments; i++)
	{
		segments[i] = new int[ysegments];
	}
	int xth = img->cols / xsegments;
	int yth = img->rows / ysegments;
		...
		for (int segx = 0; segx < xsegments; segx++)
		{
			...
			for (int y = 0; y < yth; y++)
			{
				for (int x = 0; x < xth; x++)
				{
					...
					segmentwhiteness += img->at<uchar>(ypos, xpos);

				}
			}
			segments[segx][segy] = segmentwhiteness / (255 * xth * yth);

			cout << "Segment color of: " << segx << " " << segy << "   " << segmentwhiteness << endl;
		}
	}
	int whitest = 0;
	int whitestx; int whitesty;

	for (int i = 0; i < ysegments; i++)
	{
		for (int j = 0; j < xsegments; j++)
		{
			float segcolor = segments[j][i];
			if (segcolor> whitest && segcolor > 0.001)
			{
				whitest = segcolor;
				whitestx = j;
				whitesty = i;
